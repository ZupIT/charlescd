...
class RetrieveCircleMetricsInteractorImplUnitTest extends Specification {
    ...
    def 'when no active circle found should return with zeroed value'() {
        ...
        then:
        1 * circleRepository.countGroupedByStatus(workspaceId) >> [inactiveCircleMetric]
    ...
    def 'when no inactive circle found should return with zeroed value'() {
        ...
        then:
        1 * circleRepository.countGroupedByStatus(workspaceId) >> [activeCircleMetric]
        1 * circleRepository.getNotDefaultCirclesAverageLifeTime(workspaceId) >> Duration.ofSeconds(30000)
        0 * _
    ...
    def 'when ok should return the values'() {
        ...
        then:
        1 * circleRepository.countGroupedByStatus(workspaceId) >> [activeCircleMetric, inactiveCircleMetric]
        1 * circleRepository.getNotDefaultCirclesAverageLifeTime(workspaceId) >> Duration.ofSeconds(30000)
        0 * _
