...
class RemoveMemberFromUserGroupInteractorImplTest extends Specification {

    private RemoveMemberFromUserGroupInteractor removeMemberFromUserGroupInteractor
    ...
    def "when user group does not exist should throw an exception"() {
        ...
        when:
        this.removeMemberFromUserGroupInteractor.execute(userGroupId, memberId)
    ...
    def "should not call method to remove member cause it is not present on user group"() {
        given:
        def authorId = "0a859e6c-3cdf-4b34-84d0-f9038576ac58"
        def author = getDummyUser(authorId)
        def memberId = "ccd9f717-6b38-4f1e-ad64-f735cda7a0da"
        ...
        0 * this.userGroupRepository.removeMember(_, _)
        0 * this.userGroupRepository.findPermissionsFromUserGroupAssociations(_)
        0 * this.keycloakService.disassociatePermissionsFromNewUsers(_, _)

        def exception = thrown(BusinessException)
        assert exception.message == "user.already.disassociated"

    }

    def "should call method to remove member from user group and not call keycloak service to update roles"() {
        given:
        def authorId = "0a859e6c-3cdf-4b34-84d0-f9038576ac58"
        def author = getDummyUser(authorId)
        def memberId = "ccd9f717-6b38-4f1e-ad64-f735cda7a0da"
        ...
        1 * this.userGroupRepository.findById(userGroupId) >> Optional.of(userGroup)
        1 * this.userRepository.findById(memberId) >> Optional.of(member)
        1 * this.userGroupRepository.removeMember(_, _) >> { arguments ->
    ...
    def "should call method to remove member from user group and call keycloak service to update roles"() {
        given:
        def authorId = "0a859e6c-3cdf-4b34-84d0-f9038576ac58"
        def author = getDummyUser(authorId)
        def memberId = "ccd9f717-6b38-4f1e-ad64-f735cda7a0da"
        ...
        1 * this.keycloakService.disassociatePermissionsFromNewUsers(member, workspaceAndRoles)
