...
@Named
class DeleteBuildByIdInteractorImpl(private val buildRepository: BuildRepository) : DeleteBuildByIdInteractor {
    override fun execute(id: String, applicationId: String) {
        val build = findBuild(id, applicationId)

        if (!build.canBeArchived()) {
            throw BusinessException.of(MooveErrorCode.DELETE_DEPLOYED_BUILD)
        }
