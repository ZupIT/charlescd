...
class KeycloakServiceGroovyUnitTest extends Specification {
    ...
    def 'should find group by id'() {
        ...
        def user = new User("fake-member-id", "fake-name", "fake-email", "fake-photo", [], LocalDateTime.now())
    ...
    def "should get user groups"() {
        ...
        def User = new User(id, firstName, email, "", [], LocalDateTime.now())
        def user = new UserRepresentation()
        user.id = id
