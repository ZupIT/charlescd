...
public class RetryBuildStatusNotificationInteractorTest {
	...
	public void testBuildWithoutFailureStatus() {
		List<BuildEntity> list = new ArrayList<>();
		MockBasePanacheQuery.MockBuildPanacheQuery object = new MockBasePanacheQuery.MockBuildPanacheQuery(list);
		MockBasePanacheQuery.MockBuildPanacheQuery mockBuild = Mockito.spy(object);

		when(buildRepository.findBuildsToNotify()).thenCallRealMethod();
		when(buildRepository.find("callback_status = ?1", CallbackStatus.FAILURE.name())).thenReturn(mockBuild);
		when(mockBuild.list()).thenReturn(list);

		var interactor = new RetryBuildStatusNotificationInteractorImpl(buildRepository, moduleRepository,
				notificationService);
	...
	public void testBuildWithFailureStatus() {
		...
		List<BuildEntity> list = new ArrayList<>();
		MockBasePanacheQuery.MockBuildPanacheQuery object = new MockBasePanacheQuery.MockBuildPanacheQuery(list);
		MockBasePanacheQuery.MockBuildPanacheQuery mockBuildPanacheQuery = Mockito.spy(object);
		...
		when(mockBuildPanacheQuery.list()).thenReturn(list);
		...
		when(moduleRepository.find(eq("build_id = ?1"), anyString())).thenAnswer(invocationOnMock -> {
			...
			if (buildId.equals(buildId1)) {
				...
				MockBasePanacheQuery.MockModulePanacheQuery module = new MockBasePanacheQuery.MockModulePanacheQuery(
						listModule);
				MockBasePanacheQuery.MockModulePanacheQuery mockModulePanacheQuery = Mockito.spy(module);

				when(mockModulePanacheQuery.list()).thenReturn(listModule);

				mockModulePanacheQuery.add(moduleId1, "4f94780e-ebe1-4d4c-8a97-e3b01997492a", "module_1", "tag_1",
						buildId1, registryConfigurationId, "http://registry/test");
			...
			if (buildId.equals(buildId2)) {
				...
				mockModulePanacheQuery.add(moduleId2, "5ffa9132-0569-4216-9037-824f9d0f2755", "module_2", "tag_2",
						buildId2, registryConfigurationId, "http://registry/test");
				return mockModulePanacheQuery;
			}
		...
		verify(notificationService, times(2)).notify(buildEntityArgumentCaptor.capture(),
				modulesListArgumentCaptor.capture());
