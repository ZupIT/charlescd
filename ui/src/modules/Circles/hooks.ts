...
export const useCircleQuery = (id: string, isEditing: boolean) => {
  return useQuery(['getCircleById', id], () => findCircleByIdQuery(id), {
    enabled: isEditing
  });
};

export const useCircleComponentsQuery = (id: string, hasDeploy: boolean) => {
  return useQuery(['getCircleComponents', id], () => findComponentsQuery(id), {
    enabled: hasDeploy
  });
};
