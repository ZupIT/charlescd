...
import { ModuleDeploymentEntity, DeploymentEntity, ComponentDeploymentEntity } from '../entity'
...
export class DeploymentsStatusManagementService {
    ...
    public async deepUpdateDeploymentStatusByDeploymentId(deploymentId: string, status: DeploymentStatusEnum) {
        ...
        await this.deploymentsRepository.update(deployment.id, {status: status})
    ...
    public async deepUpdateModuleStatus(module: ModuleDeploymentEntity, status: DeploymentStatusEnum) {
        await this.moduleDeploymentRepository.update(module.id, {status: status})
        return Promise.all(
            module.components.map(c => 
            this.componentDeploymentRepository.update(c.id, {status: status})))
    }
